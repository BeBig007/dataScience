...
def chart_box_no_outlier_plot(data: pd.DataFrame):
    ...
    try:
        assert isinstance(data, pd.DataFrame), "arg must be a dataframe"
        plt.figure()
        plt.boxplot(data, sym='', vert=False, 
                    patch_artist=True, 
                    boxprops=dict(facecolor='lightgreen', edgecolor='black'))
        plt.xlabel('price')
        plt.yticks([])
...
def chart_box_plot_av_basket(data: pd.DataFrame):
    ...
    try:
        assert isinstance(data, pd.DataFrame), "arg must be a dataframe"
        plt.figure()
        plt.boxplot(data['average_price'], vert=False, whis=0.3,
                    patch_artist=True, boxprops=dict(facecolor='lightblue', edgecolor='black'))
        plt.yticks([])
        plt.margins(x=0.05,y=-0.4)
        plt.tight_layout()
...
# docker cp postgres:app/box_plot_chart.png . && docker cp postgres:app/chart_box_no_outlier_plot.png . && docker cp postgres:app/average_basket_chart.png .
